...
def contact_us():
    if request.method == 'POST':
        ...
        msg = Message(subject,
                      ...
                      recipients=['info@greenifyzen.com', 'hari@greenifyzen.com', 'kumaran@greenifyzen.com', 'dhiraaj@greenifyzen.com'])
...
def get_documents():
    ...
    for invoice_id, invoice_data in invoices.items():
        if isinstance(invoice_data, dict):
            # Retrieve documentSummary for the invoice
            document_summary = invoice_data.get('documentSummary', [])

            # Ensure documentSummary is not empty and then fetch the uuid
            if document_summary:
                uuid = document_summary[0].get('uuid')

                if uuid:
                    # Trigger the GET_SUBMISSION API to update invoice data
                    submission_url = f"{os.getenv('GET_SUBMISSION')}/{uuid}"
                    headers = {
                        'Authorization': f'Bearer {auth_token}',
                        'Content-Type': 'application/json'
                    }
                    ...
                    if response.status_code == 200:
                        ...
                        # Update the MongoDB document for the invoice with new data
                        user_collection.update_one(
                        ...
                        # Update the local invoice_data for the current loop
                        invoice_data.update(submission_details)

            # Make sure invoice_data is a dictionary before calling .get()
            if isinstance(invoice_data, dict):
                # Extract the overall status from the updated invoice
                overall_status = invoice_data.get("overallStatus", "Pending")

                # Iterate over document summaries within the invoice
                for doc in invoice_data.get("documentSummary", []):
                    documents.append({
                        "document_code": doc.get("internalId"),
                        "category": doc.get("typeName"),
                        "status": overall_status,  # Use updated overallStatus
...
def get_doc_info():
            ...
            for doc in invoice_data.get("documentSummary", []):
                if doc.get("uuid") == uuid:
                    documents.append({
                        "document_code": doc.get("internalId"),
                        "category": doc.get("typeName"),
                        "status": invoice_data.get("overallStatus", "Pending"),
                        "validation_time": doc.get("dateTimeValidated", "N/A"),
                        "uuid": doc.get("uuid"),
                        "submissionUid": invoice_data.get("submissionUid"),
    ...
    supplier_info = {
        ...
        "msic_code" : invoice_data[0].get("AccountingSupplierParty", [{}])[
            0].get("Party", [{}])[0].get("MSIC", "Unknown")
    ...
    status = invoice_data[0].get("status", "Unknown")
    ...
    # Render the HTML page with the combined information
    return render_template('invoice_preview1.html', combined_info=combined_info, invoice_details= invoice_details)
...
def update_document_status(uuid):
    ...
    payload = {"status": "cancelled", "reason": "Document cancelled by user"}
    ...
    if response.status_code == 200:
        ...
        # Extract the status from the submission_details and update the overallStatus
        status = submission_details.get("status")
        ...
        user_collection.update_one(
            {"sup_email": user_email, f"invoices.{invoice_id}": {"$exists": True}},
            {
                "$set": {
                    ...
                    f"invoices.{invoice_id}.overallStatus": status,
                    # Update the whole invoice data
                    f"invoices.{invoice_id}": submission_details
                }
            }
        )
        return jsonify({"message": "Document cancelled and updated successfully."}), 200
    else:
...
def creditNote():
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            file_name = f'creditNote_{invoice_code}.json'
            try:
            ...
            return render_template('creditNote.html', supplier_info=supplier_info, sup_tin=sup_tin, sup_reg_no=sup_reg_no)
        ...
        return redirect(url_for('creditNote'))
...
def debitNote():
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            invoice_data = {
                ...
                "Invoice": [{
                    ...
                    "InvoiceTypeCode": [{"_": "03", "listVersionID": "1.1"}],
                    "ID": [{"_": invoice_code}],
                    "IssueDate": [{"_": datetime.now().strftime("%Y-%m-%d")}],
                    "IssueTime": [{"_": datetime.now().strftime("%H:%M:%SZ")}],
                    "DocumentCurrencyCode": [{"_": request.form.get("invoice_currency")}],
            ...
            # Save the combined invoice data as a JSON file
            file_name = f'debitNote_{invoice_code}.json'
            try:
                with open(file_name, 'w') as json_file:
                    json.dump(invoice_data, json_file, indent=4)
                print(f"Invoice data saved to {file_name}.")
        ...
        print(f"An unexpected error occurred: {e}")
        flash('An unexpected error occurred while processing your request. Please try again later.', 'error')
        return redirect(url_for('debitNote'))
